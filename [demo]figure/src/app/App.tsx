            ...
            return (
              ...
              >
                ...
                <div className="flex items-center justify-between h-full pl-[24px] pr-[12px]">
                  <div className="flex items-center gap-[4px] flex-1 min-w-0 h-[20px]">
                    {table.type === 'figure' ? (
                      <svg className={`shrink-0 size-4 ${isTableDisabled ? 'text-[#B2B4B4]' : 'text-[#656969]'}`} viewBox="0 0 13.3333 12" fill="currentColor">
                        <path d="M1.333 1.333h10.667v9.334H1.333V1.333zm1.334 1.334v6.666h8V2.667H2.667zM1.333 12h10.667v.667H1.333V12zM4 8.667l2-2.667 1.333 1.333L9.333 5 11.333 8.667H4z" />
                      </svg>
                    ) : (
...
export default function App() {
  const [programs, setPrograms] = useState<ProgramItem[]>([
    {
      ...
      tables: [
        ...
        { id: 'f1', name: 'Figure 14.1.1', status: 'pending', type: 'figure' as const },
        { id: 'f2', name: 'Figure 14.1.2', status: 'pending', type: 'figure' as const },
      ],
    },
  ]);
  ...
  const adaptiveWidths = useMemo(() => {
    ...
    const codeAvailable = (tw: number, sw: number) => layoutWidth - tw - sw - chrome;

    if (shellActive) {
      if (codeAvailable(treeW, shellW) < C.code.min) {
        shellW = Math.max(C.shell.minWidth, layoutWidth - treeW - chrome - C.code.min);
      }
      if (codeAvailable(treeW, shellW) < C.code.min && treeListOpen) {
        treeW = Math.max(C.treeList.min, layoutWidth - shellW - chrome - C.code.min);
      }
    }
  ...
  // Helper to get selected item type
  const getSelectedItemType = (): 'figure' | 'program' | null => {
    if (!selectedId) return null;
    for (const p of programs) {
      if (p.id === selectedId) return 'program';
      const item = p.tables.find(t => t.id === selectedId);
          ...
          <div className="flex flex-1 overflow-hidden">
            ...
            <div ref={containerRef} className={`flex-1 overflow-hidden min-w-0 ${panelLayout === 'vertical' ? 'flex flex-col' : 'flex flex-row'}`}>
              ...
              {codeOpen && (
                ...
                >
                  <CodePanel selectedItem={getSelectedItemName()} isCollapsed={false} isLocked={isSelectedItemLocked()} aiRevision={0} />
                </div>
              )}
            </div>
