...
This document details the design specifications and interaction rules for the Listing view, including global parameters, freeze & break rules, metadata sync, and print previews.

---

## 1. 全局规范 (Global Specifications)

### 首页卡片文案
- 首页卡片的进度文案统一由 `8/8 Tables Completed` 改为 `14/15 TLF Completed`

### Listing 页面规范
- **无 Group view**：移除Table View / Group View 切换 Tab 栏
- **默认上下视图**：双栏模式下，默认采用上下分屏布局（Vertical Layout）。
- **无 Program code**

---

## 2. Listing Column Freeze & Page Break Interaction (原有 Freeze & Break 规则)
...
### 全局交互规则 (Global Interaction Rules)
...
### 拖动联动与冲突解决 (Dragging & Conflict Resolution)
...
## 3. 配置项和 Metadata 的联动 (Configuration & Metadata Linkage)

- **实时联动更新**：在 Shell preview 中对 Freeze 列或 Page Break 进行任何交互式修改（如拖拽调整、删除或新增等操作）时，其对应的配置参数（例如 `freeze_cols`、`page_cols`等数值）必须实时、同步更新到 Metadata 面板中，确保两边数据的一致性。

---

## 4. 打印 Preview (Print Preview)
