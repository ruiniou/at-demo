...
import { Lock, Unlock, Table as TableIcon, Search, Filter, Sparkles, ChevronRight, ChevronLeft, ArrowLeft, Folders } from 'lucide-react';
...
import categorySvgPaths from '../imports/Category/svg-1dgvh63ado';
...
function CategoryFilter() {
  ...
  const categories = [
    { id: 'all' as CategoryType, label: 'All', icon: null },
    {
      ...
      label: 'Table',
      icon: categorySvgPaths.p2304c200
    },
    {
      id: 'listing' as CategoryType,
      label: 'Listing',
      icon: categorySvgPaths.p32eae080
    },
    {
      id: 'figure' as CategoryType,
      label: 'Figure',
      icon: categorySvgPaths.p1c14f600
    },
  ];
...
function ViewToggleBar({ treeListOpen, onToggleTreeList }: { treeListOpen: boolean; onToggleTreeList: () => void }) {
    ...
    <div className="h-[48px] shrink-0 w-full border-b-[0.6px] border-[#d8dada] flex items-center bg-white relative">
      ...
      <div className="flex items-center justify-center flex-1">
        ...
        <button
          ...
          className={`w-[120px] flex items-center justify-center gap-[4px] px-[16px] py-[8px] relative ${
        ...
        >
          ...
          <TableIcon className={`size-4 ${activeView === 'table' ? 'text-[#830051]' : 'text-[#3c4242]'}`} />
        ...
        >
          ...
          <Folders className={`size-4 ${activeView === 'group' ? 'text-[#830051]' : 'text-[#3c4242]'}`} />
          <p className={`font-['PingFang_SC:Medium',sans-serif] text-[12px] ${
...
export default function App() {
  ...
  const handleCloseAICopilot = () => {
    ...
    // Conditionally reopen Shell based on current selection
    if (selectedId && isTableSelected(selectedId)) {
      setShellPreviewOpen(true);
    }
  };
    ...
    <div className="bg-white size-full flex">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          {/* View Toggle Bar */}
          <ViewToggleBar treeListOpen={treeListOpen} onToggleTreeList={handleToggleTreeList} />
          ...
          <div className="flex flex-1 overflow-hidden">
            ...
            <div
              className="overflow-hidden shrink-0"
              style={{
                width: shellPreviewOpen ? `${shellPreviewWidth}px` : '0px',
            ...
            >
              {metadataOpen && <MetadataPanel onClose={handleCloseMetadata} />}
            </div>
            ...
            {codeOpen && (
              ...
              >
                <CodePanel selectedItem={getSelectedItemName()} isCollapsed={metadataOpen} />
              </div>
            )}
            ...
            >
              {aiCopilotOpen && <AICopilot onClose={handleCloseAICopilot} />}
            </div>

            {/* FAB for AI Copilot */}
            {!aiCopilotOpen && <FAB onClick={handleOpenAICopilot} />}
          </div>
        </div>
      </div>
