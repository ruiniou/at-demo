      ...
      <Tooltip>
        <TooltipTrigger asChild>
          ...
          >
            ...
            <div className={`overflow-clip relative shrink-0 size-[16px] ${layout === 'vertical' ? 'rotate-90' : ''}`}>
        ...
        <TooltipContent sideOffset={4} className="bg-[#3c4242] text-[#f8f7f7] border-0 shadow-[0px_2px_4px_rgba(0,0,0,0.08)] px-[6px] py-[4px] rounded-[4px]">
          <p className="font-['PingFang_SC:Regular',sans-serif] text-[12px] leading-[20px]">
            {layout === 'horizontal' ? 'Stacked view' : 'Side-by-side view'}
          </p>
        </TooltipContent>
      </Tooltip>
...
export default function App() {
  ...
  const handlePanelLayoutChange = (l: PanelLayout) => {
    setPanelLayout(l);
    setShellHeight(null);
    setShellWidth(null);
  };
  ...
  const constraints = {
    ...
    shell: { minHeight: 180, minWidth: 320, defaultWidth: 560, maxWidth: 800 },
    code: { min: 360, minHeight: 300, minWidth: 360 },
  ...
  const adaptiveWidths = useMemo(() => {
    ...
    // Accurate fixed-pixel overhead that reduces usable width
    // tree-content divider (4) + right-panel ml (3) + right-panel border (2) = 9
    // shell-code divider (4, horizontal + both open)
    // code-AI divider (4, AI open)
    const OUTER = treeListOpen && (shellPreviewOpen || codeOpen) ? 9 : 5;
    const SHELL_CODE = isHorizontal && shellPreviewOpen && codeOpen ? 4 : 0;
    const CODE_AI = codeOpen && aiCopilotOpen ? 4 : 0;
    const chrome = OUTER + SHELL_CODE + CODE_AI;

    // Desired widths clamped to hard limits
    let treeW = treeListOpen ? clamp(treeListWidth, C.treeList.min, C.treeList.max) : 0;
    let aiW = aiCopilotOpen ? clamp(aiCopilotWidth, C.aiCopilot.min, C.aiCopilot.max) : 0;

    // Shell only competes with Code for horizontal width in side-by-side layout
    const shellActive = isHorizontal && shellPreviewOpen && codeOpen;
    let shellW = shellActive
      ? clamp(shellWidth ?? C.shell.defaultWidth, C.shell.minWidth, C.shell.maxWidth)
      : 0;

    // Helper: usable width remaining for Code
    const codeAvailable = (tw: number, sw: number, aw: number) =>
      layoutWidth - tw - sw - aw - chrome;

    // Cascade compression (rule 2): shell → tree → AI, Code stays at min
    if (shellActive) {
      if (codeAvailable(treeW, shellW, aiW) < C.code.min) {
        shellW = Math.max(C.shell.minWidth, layoutWidth - treeW - aiW - chrome - C.code.min);
      }
      if (codeAvailable(treeW, shellW, aiW) < C.code.min && treeListOpen) {
        treeW = Math.max(C.treeList.min, layoutWidth - shellW - aiW - chrome - C.code.min);
      }
      if (codeAvailable(treeW, shellW, aiW) < C.code.min && aiCopilotOpen) {
        aiW = Math.max(C.aiCopilot.min, layoutWidth - treeW - shellW - chrome - C.code.min);
      }
    }

    const minTotal =
      (treeListOpen ? C.treeList.min : 0) +
      (aiCopilotOpen ? C.aiCopilot.min : 0) +
      (shellActive ? C.shell.minWidth : 0) +
      chrome +
      C.code.min;
  ...
  const [aiCopilotPrefill, setAiCopilotPrefill] = useState<string | null>(null);
  ...
  // Opens AI Copilot and prefills the input with the metadata change prompt
  const handleAddToChat = () => {
    setAiCopilotOpen(true);
    setAiCopilotPrefill('You are given metadata changes for this listing. Apply these changes to update the code accordingly.');
  };

  // Called when the user sends from AI Copilot — simulates a code update
  const handleAISend = () => {
    setAiRevision(r => r + 1);
  };
  ...
  const handleShellWidthDrag = (delta: number) => {
    setShellWidth(prev => {
      ...
      const current = prev ?? constraints.shell.defaultWidth;
      return Math.max(constraints.shell.minWidth, Math.min(
        Math.min(constraints.shell.maxWidth, containerW - constraints.code.min - 1),
        current + delta
      ));
    });
  };
      ...
      <div ref={layoutRef} className="flex flex-1 overflow-auto">
        ...
        <div className="flex-1 flex flex-col overflow-hidden mt-[3px] mb-[3px] ml-[3px] rounded-[12px] shadow-[0px_1px_4px_0px_rgba(0,0,0,0.04)] border border-[#EBECEC]">
          ...
          <div className="flex flex-1 overflow-hidden">
              ...
              {shellPreviewOpen && (
                <div
                  style={panelLayout === 'vertical' ? {
                    ...
                    // Both Shell and Code visible: use cascade width with constraints
                    flex: `0 1 ${adaptiveWidths.shell}px`,
                    minWidth: `${constraints.shell.minWidth}px`,
                    maxWidth: `${constraints.shell.maxWidth}px`,
                  } : {
                    // Shell only: fill available space without maxWidth constraint
                    flex: '1 1 auto',
                    minWidth: `${constraints.shell.minWidth}px`,
                  }}
              ...
              {codeOpen && (
                <div
                  ...
                  style={panelLayout === 'vertical'
                    ...
                    : { flex: '1 0 auto', minWidth: `${constraints.code.min}px` }
                  }
                >
                  <CodePanel selectedItem={getSelectedItemName()} isCollapsed={false} isLocked={isSelectedItemLocked()} aiRevision={aiRevision} />
                </div>
              )}
            </div>
            ...
            >
              {aiCopilotOpen && <AICopilot onClose={handleCloseAICopilot} isLocked={isSelectedItemLocked()} prefillMessage={aiCopilotPrefill} onSend={handleAISend} />}
            </div>
