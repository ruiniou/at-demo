...
### [2026-09-22] Home 与 Event 详情页 Tree List 顶栏高度不一致

* **现象 (Symptom)**：
  Home 页左上角 Tree List 顶栏实际高度为 48px，低于 Event 详情页的 52px。
* **根本原因 (Root Cause)**：
  `Main.tsx` 中两页使用独立的顶栏 DOM；`HomePage` 设置 `h-[48px]`，详情页设置 `h-[52px]`，固定高度声明不一致。
* **解决方案 (Solution)**：
  仅将 Home 顶栏改为 `h-[52px]`，保留 `shrink-0`、内边距及现有交互。浏览器实测 Home 顶栏从 48px 变为 52px，`npm run build` 通过。
* **经验教训 (Takeaways)**：
  1. 跨页面同类导航区域应核对实际容器高度，不能仅比较内部图标尺寸。
  2. 局部高度修复只修改目标容器，不批量替换其他工具栏的相同尺寸。

---

### [2026-09-22] Home 收起 Tree List 后展开按钮额外占据一行

* **现象 (Symptom)**：
  Home 收起 Tree List 后，主面板上方出现独立的 48px 展开按钮行和分隔线，将大标题整体向下推移。
* **根本原因 (Root Cause)**：
  `HomePage` 在两个内容视图之前通过 `!treeListOpen` 条件渲染独立的 `h-[48px]` 工具栏，没有把展开按钮放入各视图的标题 flex 容器。
* **解决方案 (Solution)**：
  删除独立工具栏，将同一展开按钮放在 Events 标题图标左侧，并通过 `headerLeading` 插槽放入 Projects & Studies 标题图标左侧。浏览器实测两个视图的按钮、图标和标题中心线一致；Events 展开侧栏后按钮消失，标题纵向位置不变，侧栏顶栏仍为 52px；构建通过。
* **经验教训 (Takeaways)**：
  1. 导航折叠入口应复用现有标题行，不应通过额外工具栏改变内容区域的纵向位置。
  2. 调整 Home 共用导航入口时，要覆盖 Events 与 Projects & Studies 两个互斥视图，并验证展开后的入口隐藏行为。

---

### [2026-09-22] Checkbox 禁用选中态错误显示为白底灰勾

* **现象 (Symptom)**：
  Checkbox 选中或半选后禁用，显示白底灰色标记，与确认的灰底白色标记不符。
* **根本原因 (Root Cause)**：
  `CheckboxIndicator` 的 `fill` 使用 `active && !disabled`，排除了禁用态填充；标记的 `stroke` 又在禁用时切换为灰色，沿用了旧的白底灰勾规范。
* **解决方案 (Solution)**：
  填充改为由 `active` 决定，禁用时沿用 `--color-graphite-40`；勾及半选横线统一使用 `--color-white`。同步 `guidelines/ui-standards/dropdown.md`，未选中状态及禁用交互保持不变。当前工作区 5173 预览下，浏览器计算样式覆盖启用/禁用 × 未选中/选中/半选共六种状态，并校验点击回调守卫及 ARIA 属性；`npm run build` 通过。5174 指向其他 worktree，未修改该目录；浏览器视口隐藏，未完成截图验收。
* **经验教训 (Takeaways)**：
  1. 禁用态应联合核对背景、边框和标记颜色，并覆盖半选态，不能只改单一图标颜色。
  2. 多 worktree 开发时，先确认预览服务的工作目录，避免把另一份代码的页面当作验证结果。

---

### [2026-09-22] 列表固定表头引用未定义背景 Token，滚动时文字重叠

* **现象 (Symptom)**：Projects & Studies / Events 改为固定表头后，纵向滚动时行内容透过表头，与列标题重叠。
* **根本原因 (Root Cause)**：沿用的 `bg-bg-app` 和新增 `var(--color-bg-app)` 均无有效 Token 定义，`thead` 的计算背景为透明；仅验证 sticky 坐标和列对齐无法发现遮挡问题。
* **解决方案 (Solution)**：两页表头改用现有 `bg-white` / `--color-white`。Chrome 实测背景不透明，复查双向滚动截图，并通过 21 组布局检查、19 项交互断言及生产构建。
* **经验教训 (Takeaways)**：固定表头必须显式使用已定义的不透明背景；DOM 尺寸断言应结合滚动后的截图验收。
