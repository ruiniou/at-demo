...
### [2026-09-24] Event 详情页 Tree List 与工作区顶部栏高度不一致
...
### [2026-09-24] Avatar 缩写字体不一致且视觉上未垂直居中

* **现象 (Symptom)**：
  不同容器中的 Avatar 缩写字体观感不统一，字母在圆形内看起来略微偏上。
* **根本原因 (Root Cause)**：
  共享 `Avatar` 只声明了字号、字重和 `leading-none`，没有显式绑定设计系统的 `--font-body`，因此字体取决于外围继承；Inter 大写字形在严格几何居中的行盒中还会因字体度量产生轻微视觉偏上。
* **解决方案 (Solution)**：
