...
function ChevronRightIcon({ className = "size-[16px]", color = "#830051" }) {
  return (
...
function ChevronDownIcon({ className = "size-[16px]", color = "#830051" }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
function CodeIcon({ className = "size-[16px]", color = "#830051" }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
function Tag({ className = "", text = "Table.1(290-321)" }: TagProps) {
  return (
    <div className={`bg-[#f4e8ee] content-stretch flex gap-[4px] h-[20px] items-center justify-center max-w-[240px] pl-[2px] pr-[6px] relative rounded-[4px] shrink-0 select-none ${className}`}>
      <div className="overflow-clip relative shrink-0 size-[16px] flex items-center justify-center">
        <CodeIcon className="size-[12px]" color="#830051" />
      </div>
      <div className="flex flex-[1_0_0] flex-col font-['PingFang_SC',sans-serif] justify-center leading-[0] max-w-[152px] min-w-px not-italic overflow-hidden relative text-[13px] text-[#830051] text-ellipsis whitespace-nowrap">
    ...
    <div className={`flex flex-col w-full relative ${className}`}>
      ...
      <div 
        className={`content-stretch flex flex-col items-center justify-end px-[2px] relative rounded-[10px] w-full transition-all duration-200 ${
          pending 
            ? "bg-[#f4e8ee] gap-[4px] pb-[2px] pt-[8px]" 
            : ""
        }`}
        style={{
      ...
      >
          ...
          >
            ...
            >
              <div className="w-[16px] h-[16px] flex items-center justify-center shrink-0">
                {pendingExpanded ? (
                  <ChevronDownIcon className="size-[14px]" color="#830051" />
                ) : (
                  <ChevronRightIcon className="size-[14px]" color="#830051" />
                )}
              </div>
              <div className="flex flex-col font-['PingFang_SC',sans-serif] justify-center leading-[0] not-italic relative shrink-0 text-[14px] text-[#830051] text-center whitespace-nowrap">
          ...
          <div 
            className={`bg-white border-solid content-stretch flex flex-col items-start justify-center px-[10px] relative shrink-0 w-full transition-all duration-200 ${
              isMaxHeightAndNotPending 
                ? "border border-[#ebecec] py-[9px] rounded-[6px]" 
                : isFocusedAndNotPending 
                ? "border border-[#830051] drop-shadow-[0px_0px_3px_rgba(131,0,81,0.2)] py-[8px] rounded-[8px]" 
                : "border border-[#ebecec] py-[8px] rounded-[8px]"
            }`}
          >
            <div className={`content-stretch flex relative shrink-0 w-full ${isMaxHeightAndNotPending ? "gap-[12px] items-end justify-center" : "gap-[16px] items-center"}`}>
                ...
                {isMaxHeightAndNotPending ? (
                  <div className="content-stretch flex flex-[1_0_0] gap-[10px] items-start justify-between min-w-px w-full relative">
                    <textarea
                      ...
                      className="w-full t-input text-[#3c4242] placeholder-[#b2b4b4] bg-transparent border-none outline-none resize-none font-['PingFang_SC',sans-serif] text-[14px] leading-[24px] max-h-[140px] pr-[12px] overflow-y-auto chat-maxheight-textarea"
                      rows={4}
                    />
                  </div>
                ) : (
                  <div className="flex flex-1 items-center w-full">
                    ...
                    {isTypedAndNotPending && !isFocused ? (
                      ...
                      >
                        <span className="text-[#3c4242] font-['PingFang_SC',sans-serif] text-[14px] leading-[24px] whitespace-nowrap shrink-0">
                          {inputText}
                        </span>
                        <Tag />
                      </div>
                      ...
                      <input
                        ...
                        placeholder="Ask me anything..."
                        className="flex-1 t-input text-[#3c4242] placeholder-[#b2b4b4] bg-transparent border-none outline-none font-['PingFang_SC',sans-serif] text-[14px] leading-[24px]"
                      />
                    )}
              ...
              <button 
                onClick={handleSend}
                className="bg-[#830051] hover:bg-[#6a0042] transition-colors relative rounded-[4px] shrink-0 size-[24px] flex items-center justify-center cursor-pointer select-none active:scale-95 animate-none"
              >
        ...
        {pending && (
          <div className="bg-white border border-[#ebecec] border-solid content-stretch flex flex-col items-start justify-center px-[10px] py-[8px] relative rounded-[8px] shrink-0 w-full">
            <div className="content-stretch flex gap-[16px] items-center relative shrink-0 w-full">
              ...
              <button 
                onClick={handleSend}
                className="bg-[#830051] hover:bg-[#6a0042] transition-colors relative rounded-[4px] shrink-0 size-[24px] flex items-center justify-center cursor-pointer select-none active:scale-95"
              >
                <SendIcon className="size-[12px]" color="white" />
              </button>
            </div>
