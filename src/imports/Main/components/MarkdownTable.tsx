      ...
      <table className="w-full border-collapse">
        <thead>
          <tr className="border-b border-graphite-15 bg-bg-panel">
            ...
            <th className="text-left t-table py-[8px] px-[12px] whitespace-nowrap">Status</th>
          </tr>
        </thead>
        <tbody>
          {data.map((row, idx) => (
            <tr key={idx} className="border-b border-graphite-15 last:border-0">
              ...
              <td style={{ fontWeight: 400 }} className={`t-table py-[8px] px-[12px] ${shouldNoWrap(row.status) ? 'whitespace-nowrap' : 'whitespace-normal'}`}>{row.status}</td>
            </tr>
          ))}
        </tbody>
      </table>
