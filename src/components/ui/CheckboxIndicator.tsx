...
export function CheckboxIndicator({ checked = false, indeterminate = false, disabled = false, size = 16 }: {
  ...
  const disabledActive = disabled && active;
  ...
  return (
    <svg aria-hidden="true" width={size} height={size} viewBox="0 0 16 16" fill="none" className="shrink-0">
      <rect
        x={disabledActive ? "0" : "0.5"}
        y={disabledActive ? "0" : "0.5"}
        width={disabledActive ? "16" : "15"}
        height={disabledActive ? "16" : "15"}
        rx={disabledActive ? "2" : "1.5"}
        fill={disabledActive ? "var(--color-graphite-20)" : active ? color : "white"}
        stroke={disabledActive ? "none" : active || disabled ? color : "var(--color-border-default)"}
      />
